...
.else
; this is for the C256 Foenix U
    DOS_HIGH_VARIABLES      = $18A000
    DEVICE                  = $18A000       ; 1 byte - The number of the block device
    FILE_SYSTEM             = $18A001       ; 1 byte - The type of filesystem (FAT12, FAT32, etc.)
    PARTITION               = $18A002       ; 1 byte - The number of the partitions on the device
    SECTORS_PER_CLUSTER     = $18A003       ; 1 byte - The number of sectors in a cluster
    FIRSTSECTOR             = $18A004       ; 4 bytes - The LBA of the first sector on the volume
    SECTORCOUNT             = $18A008       ; 4 bytes - The number of sectors in the volume
    NUM_RSRV_SEC            = $18A00C       ; 2 bytes - The number of hidden or reserved sectors
    CLUSTER_SIZE            = $18A00E       ; 2 bytes - The size of a cluster in bytes 
    SEC_PER_FAT             = $18A010       ; 4 bytes - The number of sectors per FAT
    FAT_BEGIN_LBA           = $18A014       ; 4 bytes - The LBA of the first sector of FAT #1
    FAT2_BEGIN_LBA          = $18A018       ; 4 bytes - The LBA of the first sector of FAT #2
    CLUSTER_BEGIN_LBA       = $18A01C       ; 4 bytes - The LBA of the first cluster in the storage area
    ROOT_DIR_FIRST_CLUSTER  = $18A020       ; 4 bytes - The number of the first cluster in the root directory
    ROOT_DIR_MAX_ENTRY      = $18A024       ; 2 bytes - The maximum number of entries in the root directory (0 = no limit)
...
.else
; This is 4 the C256 Foenix U
    DOS_CURR_CLUS           = $18A02A       ; 4 bytes - The current cluster (for delete)
    DOS_NEXT_CLUS           = $18A02E       ; 4 bytes - The next cluster in a file (for delete)
    DOS_DIR_BLOCK_ID        = $18A032       ; 4 bytes - The ID of the current directory block
                                            ;   If DOS_DIR_TYPE = 0, this is a cluster ID
                                            ;   If DOS_DIR_TYPE = $80, this is a sector LBA
    DOS_NEW_CLUSTER         = $18A036       ; 4 bytes - Space to store a newly written cluster ID
    DOS_SHORT_NAME          = $18A03A       ; 11 bytes - The short name for a desired file
    DOS_DIR_TYPE            = $18A045       ; 1 byte - a code indicating the type of the current directory (0 = cluster based, $80 = sector based)
    DOS_CURR_DIR_ID         = $18A046       ; 4 byte - the ID of the first sector or cluster of the current directory
    DOS_DEV_NAMES           = $18A04A       ; 4 byte - pointer to the linked list of device names
    FDC_MOTOR_TIMER         = $18A04E       ; 2 bytes - count-down timer to automatically turn off the FDC spindle motor
    DOS_MOUNT_DEV           = $18A050       ; 1 byte - the device code of the currently mounted device

; Larger buffers
    DOS_DIR_CLUSTER         = $18A100       ; 512 bytes - A buffer for directory entries
    DOS_DIR_CLUSTER_END     = $18A300       ; The byte just past the end of the directory cluster buffer
    DOS_SECTOR              = $18A300       ; 512 bytes - A buffer for block device read/write
    DOS_SECTOR_END          = $18A500       ; The byte just past the end of the cluster buffer
    DOS_FAT_SECTORS         = $18A500       ; 1024 bytes - two sectors worth of the FAT
    DOS_FAT_SECTORS_END     = $18A900       ; The byte just past the end of the FAT buffers
    DOS_BOOT_SECTOR         = $18A900       ; A sector for holding the boot sector
    DOS_BOOT_SECTOR_END     = $18AB00
    DOS_SPARE_SECTOR        = $18AB00       ; A spare 512 byte buffer for loading sectors
    DOS_SPARE_SECTOR_END    = $18AD00
    DOS_SPARE_FD            = $18AD00       ; A spare file descriptor buffer
.endif
